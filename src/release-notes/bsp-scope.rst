...
.. blockdiag::
   ...
   blockdiag {
     ...
     meta-tqs-arm       [description = "OpenEmbedded/Yocto BSP layer for TQ Systems ARM based platforms (`this <index.html>`_)."];
     meta-fsl-arm       [description = "`OpenEmbedded/Yocto BSP layer for Freescale's ARM platforms. <https://github.com/Freescale/meta-fsl-arm>`_"];
     poky-meta          [description = "`Poky Build Tool and Metadata. <http://git.yoctoproject.org/cgit/cgit.cgi/poky/tree/meta>`_"];
     poky-meta-yocto    [description = "`Yocto Build Tool and Metadata. <http://git.yoctoproject.org/cgit/cgit.cgi/poky/tree/meta-yocto>`_"];
   }
